/*	*  LoaderEFI.c
	*  UEFI Bootloader Of SinoOS,
  	*  Current OS Version 0.02PreChannel3 ,LoaderVer 0.3
  	*  finished at 2022/8/17
  	*  Done:SysVideoModelChange,GetMemory,LoadKernel,find acpi rsdp
  	*  Compiled by edk2 efi firmware env
...
//load kernel file
	...
	status = RootFs->Open(RootFs,&FileHandle,
	(CHAR16*)L"kernel.bin",EFI_FILE_MODE_READ,0);
	if(EFI_ERROR(status))
	{
...
//set video model
	...
	for(i = 0;i < (gGraphicsOutput->Mode->FrameBufferSize + 0x200000 - 1) >> 21;i++) // map to virtual address0xffff800003000000
	{
...
//exit efi boot service & run kernel
